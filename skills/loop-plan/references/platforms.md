...
| plan dir | `~/.claude/plans/` | `.codex/loop/` in the repo | `.pi/plans/` in the repo |
...
Stack explorers shipped with the plugin for Claude Code: `android-kmp-explorer`, `swiftui-explorer`, `react-nextjs-explorer`. Other agents the user has installed may be used when their description matches the scope; never assume an agent exists without checking the platform's agent list.
